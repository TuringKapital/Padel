...
# ── Set date and wait for SVG to update ───────────────────────────────────────
async def set_date_and_wait(page, date_str):
    before = await page.evaluate(JS_GET_EVENT_COUNT)
    ...
    for _ in range(16):
        ...
        after = await page.evaluate(JS_GET_EVENT_COUNT)
        if after != before:
            log.info("SVG updated (events: %d -> %d)", before, after)
            return
        try:
