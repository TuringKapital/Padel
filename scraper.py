...
async def scrape_date(page, target_date: datetime) -> list[dict]:
    ...
    slot_data = await page.evaluate("""
        () => {
            const BOOKED = ['occupied','booked','reserved','taken','busy','azul','verde','amarelo','vermelho'];
            const FREE   = ['free','available','empty','livre','disponivel'];

            const tables = Array.from(document.querySelectorAll('table'));
            const grid   = tables.reduce((best, t) =>
                t.querySelectorAll('td').length > (best ? best.querySelectorAll('td').length : 0) ? t : best
            , null);

            if (!grid) return { error: 'no table', courts: [] };
            ...
            for (let r = 1; r < rows.length; r++) {
                ...
                cells.forEach((cell, c) => {
                    ...
                    const isBooked = BOOKED.some(b => cls.includes(b))
                                  || cell.querySelector('a,div.booking,div.event') !== null;
                    const isFree   = FREE.some(f => cls.includes(f)) || (!isBooked && cell.textContent.trim() === '');
                    if (isBooked || isFree) {
                        tallies[c].total++;
