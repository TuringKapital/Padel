...
(dateStr) => {
    ...
    if (typeof jQuery !== 'undefined') {
        jQuery('#fechaTabla').datepicker('setDate', dateStr);
        jQuery('#fechaTabla').trigger('change');
    }
    return true;
}
"""
...
# ── Set date and wait for spinner to finish ───────────────────────────────────
async def set_date_and_wait(page, date_str):
    # Trigger date change via JS
    await page.evaluate(JS_SET_DATE, date_str)

    # Also type into the field as fallback
    await page.click("#fechaTabla", click_count=3)
    await page.type("#fechaTabla", date_str, delay=30)
    await page.keyboard.press("Enter")

    # Wait for loading spinner to appear (up to 2s)
    for _ in range(4):
        await page.wait_for_timeout(500)
        is_loading = await page.evaluate(JS_IS_LOADING)
        if is_loading:
            log.info("Spinner appeared, waiting for it to finish...")
            break
    ...
    for _ in range(30):
        ...
        is_loading = await page.evaluate(JS_IS_LOADING)
        if not is_loading:
            log.info("Spinner gone, SVG ready")
            break
